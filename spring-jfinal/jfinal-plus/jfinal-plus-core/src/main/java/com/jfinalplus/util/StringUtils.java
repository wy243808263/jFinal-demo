...
public final class StringUtils {
	...
	private static boolean startsWith(final String s, final String prefix, final boolean ignoreCase) {
		if (s == null || prefix == null) { return s == null && prefix == null; }
	...
	private static boolean endsWith(final String s, final String suffix, final boolean ignoreCase) {
		if (s == null || suffix == null) { return s == null && suffix == null; }
	...
	public static int indexOfIgnoreCase(String s, char c, int startIndex, int endIndex) {
		...
		for (int i = startIndex; i < endIndex; i++) {
			if (Character.toLowerCase(s.charAt(i)) == c) { return i; }
		}
		return -1;
	}
	...
	public static String substringBefore(final String s, final String separator) {
		if (isEmpty(s) || separator == null) { return s; }
	...
	public static String substringBeforeLast(final String s, final String separator) {
		if (isEmpty(s) || isEmpty(separator)) { return s; }
	...
	public static String substringAfterLast(final String s, final String separator) {
		...
		final int pos = s.lastIndexOf(separator);
		if (pos < 0 || pos == s.length() - separator.length()) { return ""; }
		return s.substring(pos + separator.length());
	}
	...
	public static String substringBetween(final String s, final String open, final String close) {
		...
		if (start >= 0) {
			...
			if (end >= 0) { return s.substring(start + open.length(), end); }
		}
		return null;
	}
	...
	public static String mid(final String s, int pos, final int len) {
		if (s == null) { return null; }
		if (len < 0 || pos > s.length()) { return ""; }
		if (pos < 0) {
			pos = 0;
		}
		if (s.length() <= pos + len) { return s.substring(pos); }
	...
	public static String repeat(final String s, final int repeat) {
		...
		if (repeat == 1 || inputLength == 0) { return s; }
	...
	public static String repeat(final String s, final String separator, final int repeat) {
		if (s == null || separator == null) { return repeat(s, repeat); }
	...
	public static String leftPad(String str, int size, char padChar) {
		...
		if (pads > 8192) { return leftPad(str, size, String.valueOf(padChar)); }
	...
	public static String leftPad(String str, int size, String padStr) {
		...
		if (pads <= 0) { return str; }
		if ((padLen == 1) && (pads <= 8192)) { return leftPad(str, size, padStr.charAt(0)); }
		if (pads == padLen) { return padStr.concat(str); }
		if (pads < padLen) { return padStr.substring(0, pads).concat(str); }
	...
	public static String rightPad(String str, int size, char padChar) {
		...
		int pads = size - str.length();
		if (pads <= 0) { return str; }
		if (pads > 8192) { return rightPad(str, size, String.valueOf(padChar)); }
	...
	public static String rightPad(String str, int size, String padStr) {
		...
		int pads = size - strLen;
		if (pads <= 0) { return str; }
		if ((padLen == 1) && (pads <= 8192)) { return rightPad(str, size, padStr.charAt(0)); }
		if (pads == padLen) { return str.concat(padStr); }
		if (pads < padLen) { return str.concat(padStr.substring(0, pads)); }
		char[] padding = new char[pads];
		char[] padChars = padStr.toCharArray();
		for (int i = 0; i < pads; i++) {
			padding[i] = padChars[(i % padLen)];
	...
	public static String removeStart(final String s, final String remove) {
		if (isEmpty(s) || isEmpty(remove)) { return s; }
	...
	public static String removeStartIgnoreCase(final String s, final String remove) {
		if (isEmpty(s) || isEmpty(remove)) { return s; }
		if (startsWithIgnoreCase(s, remove)) { return s.substring(remove.length()); }
		return s;
	}
	...
	public static String removeEnd(final String s, final String remove) {
		if (isEmpty(s) || isEmpty(remove)) { return s; }
		if (s.endsWith(remove)) { return s.substring(0, s.length() - remove.length()); }
		return s;
	}
	...
	public static String removeEndIgnoreCase(final String s, final String remove) {
		if (isEmpty(s) || isEmpty(remove)) { return s; }
		if (endsWithIgnoreCase(s, remove)) { return s.substring(0, s.length() - remove.length()); }
		return s;
	}
	...
	public static String remove(final String s, final char remove) {
		if (isEmpty(s) || s.indexOf(remove) < 0) { return s; }
	...
	private static String replaceEach(final String text, final String[] searchList, final String[] replacementList,
		...
		if (text == null || text.isEmpty() || searchList == null || searchList.length == 0 || replacementList == null
				|| replacementList.length == 0) { return text; }

		// if recursing, this shouldn't be less than 0
		if (timeToLive < 0) { throw new IllegalStateException("Aborting to protect against StackOverflowError - "
		...
		if (searchLength != replacementLength) { throw new IllegalArgumentException(
