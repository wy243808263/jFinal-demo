...
function img_check(a, b, c) {
    ...
    0 != this.AllowExt && -1 == this.AllowExt.indexOf(this.FileExt) ? errorTips("请上传jpg、gif、png格式头像，大小不超过2M") : ($("#" + c + "_error").text("").hide(), $.ajaxFileUpload({
        url: b,
        secureuri: !1,
        fileElementId: c,
        data: {
...
function file_check(obj, action_url, id) {
    var userId;
    return 2 == uploadFlag ? !1 : (uploadFlag = 2, $("#loadingImg").css("visibility", "visible"), obj = $("#" + id), userId = $("#userid").val(), this.AllowExt = ".doc,.docx,.pdf,.ppt,.pptx,.txt,.wps", this.FileExt = obj.val().substr(obj.val().lastIndexOf(".")).toLowerCase(), "" == obj.val() ? ($("#loadingImg").css("visibility", "hidden"), !1) : (0 != this.AllowExt && -1 == this.AllowExt.indexOf(this.FileExt) ? (errorTips("只支持word、pdf、ppt、txt、wps格式文件，请重新上传"), $("#loadingImg").css("visibility", "hidden"), uploadFlag = 1) : $.ajaxFileUpload({
        type: "POST",
        url: action_url,
        secureuri: !1,
        fileElementId: id,
...
$(function() {
        ...
        $(".projectForm").each(function() {
            $(this).validate({
                ...
                messages: {
                    projectName: {
                        required: "请输入项目名称",
                        specialchar: "请输入有效的项目名称",
                        checkNum: "请输入有效的项目名称",
                        maxlenStr: "请输入有效的项目名称"
                    },
                    thePost: {
                        required: "请输入担任职务",
                        specialchar: "请输入有效的职务，如：产品负责人",
                        checkNum: "请输入有效的职务，如：产品负责人",
                        maxlenStr: "请输入有效的职务，如：产品负责人"
                    },
                    projectYearStart: {
                        required: "请选择开始时间",
                    ...
                    projectDescription: {
                        specialchar: "请输入10-500字且有效的项目描述",
                        checkNum: "请输入10-500字且有效的项目描述",
                        rangeLenStr: "请输入10-500字且有效的项目描述"
                    }
                },
            ...
            $(this).validate({
                ...
                submitHandler: function(a) {
                    ...
                    $.ajax({
                        ...
                        var c, d, e;
                        null != b.resubmitToken && "" != b.resubmitToken && $("#resubmitToken").val(b.resubmitToken),
                        b.success ? (c = "", d = "", e = b.content.workShow, 3 == b.code ? "" != e ? (c += '<li data-id="' + e.id + '"><div class="workList c7">' + '<i class="sm_del dn"></i>' + '<i class="sm_edit dn"></i>', e.url && (d = "http://" == e.url.substring(0, 7) || "https://" == e.url.substring(0, 8) ? e.url: "http://" + e.url, c += '<div class="f16"><span>网址：</span><a href="' + d + '" target="_blank">' + e.url + "</a></div>"), c += "<p>" + e.workName + "</p>" + "</div></li>", placeholderFn(), "" != $.trim(h.obj.children(".workShow").children(".slist").html()) && $(a).remove(), h.obj.children(".workShow").children(".slist").prepend(c), h.obj.children(".workShow").removeClass("dn"), h.obj.children(".workEdit").addClass("dn"), h.obj.children(".c_add").removeClass("dn")) : "" != $.trim(h.obj.children(".workShow").children(".slist").html()) ? ($(a).remove(), h.obj.children(".c_add").removeClass("dn")) : (h.obj.children(".workEdit").addClass("dn"), h.obj.children(".workAdd").removeClass("dn")) : 4 == b.code ? (c += '<div class="workList c7"><i class="sm_del dn"></i><i class="sm_edit dn"></i>', e.url && (d = "http://" == e.url.substring(0, 7) || "https://" == e.url.substring(0, 8) ? e.url: "http://" + e.url, c += '<div class="f16"><span>网址：</span><a href="' + d + '" target="_blank">' + e.url + "</a></div>"), c += "<p>" + e.workName + "</p>" + "</div>", placeholderFn(), $(a).parent("li").html(c)) : 2 == b.code && ($(a).parent("li").remove(), "" == $.trim(h.obj.children(".workShow").children(".slist").html()) && h.AddCancel), h.obj.find(".slist li:last-child").addClass("noborder"), $("#lastChangedTime span").text(b.content.refreshTime), $("#resumeScore .which div").text(b.content.infoCompleteStatus.msg), $("#resumeScore .which span").attr("rel", b.content.infoCompleteStatus.nextStage), scoreChange(b.content.infoCompleteStatus.score), k(h.obj), b.content.isNew && changeAllIds(b.content.jsonIds)) : alert(b.msg),
                        $(a).find(":submit").val("保 存").attr("disabled", !1)
                    })
                }
            })
    ...
    jQuery.validator.addMethod("resumeSpecialchar",
    function(a, b) {
        var c = /^([`~!@$^&':;,?~！……&；：。，、 ？=])/;
    ...
    $("#resumeNameForm").validate({
        ...
        messages: {
            resumeName: {
                required: "请输入简历名称",
                checkNum: "请输入有效的简历名称",
                resumeSpecialchar: "请输入有效的简历名称",
                maxlenStr: "请输入20字以内的简历名称"
            }
        },
        errorPlacement: function(a, b) {
            a.appendTo($(b).parent())
    ...
    $("#profileForm").validate({
        ...
        submitHandler: function(a) {
            ...
            $(a).find(":submit").val("保存中...").attr("disabled", !0),
            $.ajax({
                url: ctx + "/user/savePerInfo",
                type: "POST",
                data: {
                	pname: $("#pname").val(),
                    sex: c,
                    education: $("#education").val(),
                    workExperienc: $("#workExperienc").val(),
                    status: f,
                    email: $("#email").val(),
                    phone: $("#phone").val(),
                    id: $("#id").val(),
    ...
    $("#expectForm").validate({
        ...
        submitHandler: function(a) {
            ...
            $.ajax({
                ...
                if (null != g.resubmitToken && "" != g.resubmitToken && $("#resubmitToken").val(g.resubmitToken), g.success) {
                    if (c || d || e || f) {
...
//        "" != a && suggestPosition(a, ".positionName")
        ...
        $(this).validate({
            ...
            submitHandler: function(a) {
                ...
                $.ajax({
                    ...
                    var d, e, f;
                    if (null != b.resubmitToken && "" != b.resubmitToken && $("#resubmitToken").val(b.resubmitToken), b.success) {
                        for (d = b.content.workExperiences, e = "", f = 0; f < d.length; f++) e += 0 == f % 2 ? '<li data-id="' + d[f].id + '">': '<li data-id="' + d[f].id + '">',
        ...
        $(this).validate({
            ...
            submitHandler: function(a) {
                ...
                $.ajax({
                    ...
                    var c, d, e;
                    if (null != b.resubmitToken && "" != b.resubmitToken && $("#resubmitToken").val(b.resubmitToken), b.success) {
                        for (c = b.content.educationExperiences, d = "", e = 0; e < c.length; e++) d += 0 == e % 2 ? '<li data-id="' + c[e].id + '">': '<li data-id="' + c[e].id + '">',
    ...
    $(".descriptionAdd span,.descriptionAdd").click(function() {
        $(".descriptionForm span.error").hide()
    }),
    $(".descriptionForm").validate({
        ...
        messages: {
            selfDescription: {
                specialchar: "请输入10-500字的自我描述",
                checkNum: "请输入10-500字的自我描述",
                rangeLenStr: "请输入10-500字的自我描述"
            }
        },
        submitHandler: function(a) {
