...
$(function(){
	 var $div_li = $(".position_head li"); 
	 var index = $div_li.index(this) ;
	 if($div_li.length == 1){
	 $("#position_recommend>div").eq(index+2).show() ;
	 $("#similar_footer").hide() ;
	 }
	    $div_li.click(function(){
	    	if($div_li.length > 1){
	    		$(this).addClass("guess_selected").siblings().removeClass("guess_selected") ;
	 			var index = $div_li.index(this) ;
	 			$("#position_recommend>div").eq(index).show().siblings("div").hide() ;
	    	}else{
			  $(this).removeClass(guess_selected) ;
			}
		})
})